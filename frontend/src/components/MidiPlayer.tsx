...
export default function MidiPlayer({ midiUrl, bpm = 120 }: MidiPlayerProps) {
  ...
  const loadMidi = async () => {
    try {
      ...
      // For now, we'll show a visual interface since direct MIDI playback
      // requires additional libraries or conversion to audio
      // In production, you'd convert MIDI to audio or use a MIDI library

      // Estimate duration from BPM and file size (rough estimate)
      // In production, parse MIDI file for accurate duration
