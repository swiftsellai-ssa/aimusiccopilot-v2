...
const MusicPlayer: React.FC<MusicPlayerProps> = ({ midiUrl, bpm = 120, instrument = 'full' }) => {
  ...
  // Fix applied: drums defined as MembraneSynth, not Sampler
            ...
            if (note.midi < 48 || track.channel === 9) {
              if (synthsRef.current.drums) {
                const drumNote = note.midi === 36 ? "C1" :
                  note.midi === 38 ? "D1" :
                    note.midi === 42 ? "F#1" : "C2";
                synthsRef.current.drums.triggerAttackRelease(drumNote, "8n", time, velocity);
              }
            } else if (note.midi < 60) {
              synthsRef.current.bass?.triggerAttackRelease(note.name, note.duration || "8n", time, velocity);
            } else {
              synthsRef.current.lead?.triggerAttackRelease(note.name, note.duration || "4n", time, velocity * 0.7);
            }
          }, note.time);
        });
      });
